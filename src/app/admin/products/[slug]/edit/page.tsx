...
export default function EditProductPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    fetch(`/api/products/${slug}`)
      ...
      .then(data => {
        if (data.product) {
          ...
          setValue('width', p.dimensions?.width || '');
          setValue('height', p.dimensions?.height || '');
          setValue('depth', p.dimensions?.depth || '');
          setValue('weight', p.dimensions?.weight || '');
          setValue('featured', p.featured || false);
          setTags(p.tags || []);
          setImages(p.images || []);
        } else {
  ...
  if (isFetching) return <div className="p-8 font-sans text-brand-gray">Loading product details...</div>;
    ...
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="font-sans text-sm text-brand-gray">Update product details</p>
        </div>
      </div>
